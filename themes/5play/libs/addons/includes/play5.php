...
$types			= 'post';
$sources		= '5play.ru';
$demos			= 'https://5play.ru/en/7591-toca-life-world.html'; 
?>
            ...
            if(stristr($url, $sources)) {
				...
				if($duplicatepost){
					$post_args = array(
                        ...
                        'post_name'		=> sanitize_title_with_dashes(ex_themes_clean($gets_data['title'])),
				...
				if (empty($check_title) ){
                    ...
                    $terms				= array();
                    foreach($gets_data['genres_web'] as $term):
                    ...
                    add_post_meta( $post_id, 'wp_images_GP', $gets_data['poster_web'] );
                    ...
                    add_post_meta( $post_id, 'wp_genres_GP', $gets_data['genres_web'] ); 
                    wp_set_object_terms( $post_id, $gets_data['genres_web'], 'category', true );
                    ...
                    ##########   Upload poster
                    $image_url					= $gets_data['poster_web'];
					...
					##########
                    $tags				= array(''.$gets_data['title'].' '.$gets_data['version_GP'].',');
                    wp_set_object_terms( $post_id, $gets_data['genres_GP'], 'post_tag', true );
                    wp_set_object_terms( $post_id, $gets_data['title'], 'post_tag', true );
        ...
        <div class="tab-content">
			...
			<div class="play_menu" style="text-transform:uppercase"><b>Latest Uploads Apps</b></div>                 
                    ...
                    function latest_apps() {
						...
						include_once 'dom.php';
						$target1	= 'https://5play.ru/en/android/apps/';
                        $htmls		= file_get_contents($target1, false, stream_context_create($ssl)); 
                        $html		= file_get_html($target1, LIBXML_NOERROR, stream_context_create($ssl)); 
                        foreach($html->find("div.entry.entry-app") as $article) {
						$item['title']		= trim($article->find('a', 0)->{'plaintext'});
						$item['url']		= trim($article->find('a', 0)->href);
						$item['imgs']		= trim($article->find('img', 0)->{'src'});	
						$item['imgs']		= str_replace(array('/cdn-cgi/', '/mirage/', '/8c122c98718af9d55a5fc39e956b293dd61cae0124eb7260d22eb13a2f4a7cba', '/6b7a1117181e1cf1a19b7aada9020b129b14bd12e50824a5fadbdf2d080ecf34','/1440','/1280/','miragemirage', 'mirage/'), '', $item['imgs']);
						$item['img']		= 'https://5play.ru/'.$item['imgs'];
						$apps[]				= $item;
						}
                        $html->clear();
                        unset($html);
					...
					<?php 
					$count		= 1;
					foreach($apps as $GP_) { if ($GP_['url']) { ?>					
					<li class="table-row"> 
			...
			<div class="play_menu" style="text-transform:uppercase"><b>Latest Uploads Games </b></div>               
                    ...
                    function latest_games() {
						...
						require_once 'ssl.php';	
						include_once 'dom.php';
						$target1	= 'https://5play.ru/en/android/games/'; 
                        $htmls		= file_get_contents($target1, false, stream_context_create($ssl)); 
                        $html		= file_get_html($target1, LIBXML_NOERROR, stream_context_create($ssl)); 
                        foreach($html->find("div.entry.entry-app") as $article) {
						$item['title']		= trim($article->find('a', 0)->{'plaintext'});
					...
					<?php 
					$count		= 1;
					foreach($games as $GP_) { if ($GP_['url']) { ?>
					<li class="table-row"> 
						<div class="col col-1" data-label="Image"><img style="width:50px; height:50px;" src="<?php echo $GP_['img']; ?>"></div>
						<div class="col col-2" data-label="Title"><h3 style="font-size: 1.7rem;"><?php echo $GP_['title']; ?></h3></div>
						<div class="col col-3" data-label="Submit"><form name="myForm" id="myForm" method="POST" class="sidebar" style=""><input style="display:none" type="text" name="wp_url" value="<?php echo $GP_['url']; ?>"><input type="submit" id="submit_<?php echo $count; ?>" name="wp_sb" class="button-primary abe2" value="<?php echo postnow; ?>"></form></div>
			...
			<li>Open website <a style="color:#1e73be" href="https://5play.ru/en/" target="_blank"><?php echo $sources; ?></a></li>
