...
function wp_modder() {
$sources        = 'modder.me';
$demos          = 'https://modder.me/roblox-mod-menu-apk.html';
?>
        ...
        <div class="tab-content">
			...
			<section>
				<div class="row">
					...
					ini_set('display_errors', 'off');
						$target1	= 'http://modder.me/games'; 
                        $html		= file_get_html($target1, LIBXML_NOERROR);
                        foreach($html->find("//div.row div[@class='col-12 col-md-6 col-xl-4 mb-4']") as $article) {
						$item['title']		= trim($article->find('h3.h6', 0)->{'plaintext'});
						$item['url']		= trim($article->find('a', 0)->href);
						$item['img']		= trim($article->find('img', 0)->{'src'});
						$item['versi']		= trim($article->find('div.text-center.flex-shrink-0.ml-auto div.small.text-truncate.text-muted', 0)->{'plaintext'});
						$item['mod']		= trim($article->find('div.small.text-truncate.text-muted span.align-middle', 1)->{'plaintext'});
						$ret[]				= $item;
						}
                        $html->clear();
                        unset($html);
			...
			<section>
				<div class="row">
					...
					/*<div class="small text-truncate text-muted" style="width: 4rem;">
					https://www.php.net/manual/en/function.base64-encode.php#115261
					*/
					if($GP_['img']){
					$image			= $GP_['img']; 
