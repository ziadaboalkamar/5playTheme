...
class starting_now{	
	...
	public function scrape_web_info($sources) {	
		...
		$arr['id_ps']					= $this->match('/<a.*?href=".*?\?id=(.*?)\&.*?".*?<svg class="icon playstore-icon">.*?<\/a>/ms', $links, 1);
		
		$arr['title_id']				= $this->match('/<a.*?title="View on Play Store".*?href=".*?\?id=(.*?)\&.*?".*?>.*?<svg class="icon playstore-icon">.*?<\/svg>.*?<\/a>/ms', $links, 1);
		$arr['title_id_alt']			= $this->match('/<a.*?title="View on Play Store".*?href=".*?\?id=(.*?)\&.*?".*?>.*?<svg class="icon playstore-icon">.*?<\/svg>.*?<\/a>/ms', $links, 1);
		
		$arr['GP_ID']					= $this->match('/<a.*?title="View on Play Store".*?href=".*?\?id=(.*?)\&.*?".*?>.*?<svg class="icon playstore-icon">.*?<\/svg>.*?<\/a>/ms', $links, 1);
		$arr['GP_ID_alt']				= $this->match('/<a.*?title="View on Play Store".*?href=".*?\?id=(.*?)\&.*?".*?>.*?<svg class="icon playstore-icon">.*?<\/svg>.*?<\/a>/ms', $links, 1);
		
		$arr['GP_IDX']					= $this->match('/<a.*?title="View on Play Store".*?href=".*?\?id=(.*?)\&.*?".*?>.*?<svg class="icon playstore-icon">.*?<\/svg>.*?<\/a>/ms', $links, 1);
		$arr['GP_IDX_alt']				= $this->match('/<a.*?title="View on Play Store".*?href=".*?\?id=(.*?)\&.*?".*?>.*?<svg class="icon playstore-icon">.*?<\/svg>.*?<\/a>/ms', $links, 1);
