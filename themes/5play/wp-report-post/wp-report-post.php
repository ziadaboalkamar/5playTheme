...
function count_reports() {
	global $wpdb;
	$wpdb->get_results( "SELECT meta_value, post_id FROM ".$wpdb->prefix."postmeta WHERE meta_key = '_wp_report_post' ORDER BY meta_id DESC" );
	return $wpdb->num_rows;
}
function toolbar_admin_reports( $wp_admin_bar ) {
	$args = array(
		'id'    => 'menu_reports',
		'title' => '<span class="ab-icon dashicons dashicons-megaphone"></span> '.count_reports().' ',
		'href'  => admin_url().'admin.php?page=wp-report-post',
...
class WP_Report_Post_2_Baru{
    ...
    function enqueue_scripts() { 
        $dirs				= EX_THEMES_URI.'/wp-report-post/';
        wp_enqueue_style( THEMES_NAMES.'-wp-report-post', $dirs.'/css/style.css' );	
        wp_enqueue_style( THEMES_NAMES.'-remodal', $dirs.'/lib/remodal/remodal.css' );	
        wp_enqueue_script( THEMES_NAMES.'-remodal', $dirs.'/lib/remodal/remodal.js', array('jquery'), null, false);
...
<?php
    ...
    function admin_menu() {
        ...
        add_menu_page( __('Reported Apps', 'wp-report-post'), $count_reports ? sprintf( 'Report Apps <span class="awaiting-mod">%d</span>', $count_reports ) : 'Report Apps', 'edit_others_posts', 'wp-report-post', array($this, 'reported_posts'), 'dashicons-megaphone',  );
    ...
    function reported_posts()    {
        $this->import_old();
        $reports = new WP_Report_Post_List_Baru();
...
<div class="wrap">
    ...
    <h2><?php _e('WP Report Apps Options', 'wp-report-post'); ?></h2>
...
<?php
    ...
    function __construct()
    {
        ...
        (
            ...
            'email_address'         => '',
            'email_subject'         => __('[WP Report Post] Somebody reported a Apps!', 'wp-report-post'),
            'email_body'            => __("Hey, Admin!\nSomebody has just reported a Apps.\nTitle: [post_title]\nLink: [permalink]\nPost ID: [post_id]\nReporter Name: [name]\nReporter Email: [email]\nReporter IP: [ip]\nMessage: [message]\n--\nSincerely, WP Report Post Plugin", 'wp-report-post'),
            'msg_min_length'        => '16',
            'use_captcha'           => '0',
            'show_ip_to_client'     => '0',
            'recaptcha_sitekey'     => '',
            ...
            'text_report_post'      => __('Report Apps', 'wp-report-post'),
            'text_report_link'      => __('Report Apps', 'wp-report-post'),
            ...
            'text_post_doesnt_exist'=> __('Specified Apps does not exist', 'wp-report-post'),
            ...
            'text_msg_invalid'      => __('Please describe why do you think this Apps is inappropriate', 'wp-report-post'),
            'text_already_reported' => __('You have already reported this Apps', 'wp-report-post'),
            'text_success'          => __('You have successfully reported inappropriate Apps', 'wp-report-post'),
            'text_error'            => __('Error submitting report', 'wp-report-post'),
            'text_require_login'    => __('Please <a href="%s">log in</a> to report Apps', 'wp-report-post'),
        ...
        (
            ...
            'text_post_doesnt_exist'=> __('Invalid Apps', 'wp-report-post'),
...
// Reported posts list
class WP_Report_Post_List_Baru extends WP_List_Table
{
    ...
    //
    function format_reports_alt($reportss)
    {
        $reps = maybe_unserialize($reportss);
        if (is_array($reps))
        {
            $htmls = '';
            foreach ($reps as $rows)
            {                 
                $htmls .=  esc_html($rows['msg']);                 
            }
            return $htmls;
        }
        else
    ...
    function column_default($item, $column_name)
    {
        switch($column_name)
        {
            ...
            case 'post_title':
                ...
                break;
            case 'meta_value':
                return $this->format_reports_alt($item[$column_name]);
                break;
            case 'post_status':
    ...
    function column_post_title($item)
    {
        ...
        return sprintf('%1$s %2$s',
            /*$1%s*/ sprintf('<a href="%s" title="%s" target="_blank">%s</a><br>  ', get_post_permalink($item['post_id']), __('View post in new tab', 'wp-report-post'), $item['post_title']),
    ...
    function get_columns()
    {
        $columns = array(
            ...
            'post_title' => __('Title ', 'wp-report-post'),
            'post_date'  => __('Date ', 'wp-report-post'),
            'post_author'=> __('User', 'wp-report-post'),
            /* 'post_status'=> __('Post Status', 'wp-report-post'), */
            'meta_value' => __('Report Message', 'wp-report-post')
        );
        return $columns;
    }
    //
...
$_wp_report_post_2 = new WP_Report_Post_2_Baru;