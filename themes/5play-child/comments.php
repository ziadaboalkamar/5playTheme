...
<?php if ( comments_open()) { ?>
    <form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" method="post" name="dle-comments-form" id="dle-comments-form">
        ...
        <div id="addcomment" class="block b-add-comments ignore-select">
            <div class="b-head">
                <h3 class="section-title"><i class="s-purple c-icon"><svg width="24" height="24"><use xlink:href="#i__addcom"></use></svg></i><?php global $opt_themes; if($opt_themes['exthemes_comment_Comments']) { ?>  <?php echo _e($opt_themes['exthemes_comment_Comments'],CHILD_THEME); ?><?php } ?></h3>
            </div>
            ...
            <div class="b-cont">
                ...
                function my_comment_form_defaults( $defaults ) {
                    ...
                    $defaults['comment_notes_before'] = '<div class="form-group" id="comment-editor"><label class="c-muted" for="comments">'. esc_html__( 'Yours email address will not be published. Required fields are marked *', CHILD_THEME ).'</label></div>';
                ...
                function wc_comment_form_change_cookies( $fields ) {
                    ...
                    $fields['cookies'] = '<div class="form-group" id="comment-editor">' . '<label class="c-muted" for="comments"><input id="wp-comment-cookies-consent" name="wp-comment-cookies-consent" type="checkbox" value="yes"' . $consent . ' />&nbsp;&nbsp;'.esc_html__('Save my name, email, and website in this browser for the next time I comment.', CHILD_THEME).'</label></div>';
                    return $fields;
                }
                $comment_args = array(
                    'title_reply'		=> '',
                    'fields'			=> apply_filters('comment_form_default_fields', array(
                        'author'			=> '<div class="form-combo">'.($req ? ' ' : '').'<div class="form-group"><input  placeholder="'.esc_html__('Your Name',CHILD_THEME).'" class="form-control" id="author" name="author" type="text" value="'. esc_attr($commenter['comment_author']).'"  rows="5"  /></div>',
                        'email'				=> ($req ? ' ' : '').'<div class="form-group"><input placeholder="'.esc_html__('Your Email',CHILD_THEME).' " class="form-control" id="email" name="email" type="text" value="'.esc_attr($commenter['comment_author_email']).'"  rows="5"  /></div></div>',
                        'url'				=> '')),
                    'comment_field'		=> '<!--<label for="comment">'.__('Let us know what you have to say:') . '</label>-->'.'<div class="form-group" id="comment-editor"><div class="bb-editor"><textarea placeholder="'.esc_html__('Your Comment',CHILD_THEME).'" name="comment" id="comment" class="apkmody materialize-textarea has-very-light-gray-background-color comment-form-input no-border" rows="5"></textarea></div></div>',
