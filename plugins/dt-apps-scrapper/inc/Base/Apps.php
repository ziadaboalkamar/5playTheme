...
//                }
            ...
            if ($contents === false) {
                // Unable to fetch the logo image from the provided URL
                $error_message = "Failed to fetch the logo image from the provided URL: " . $logo_url;
                // You can log or handle the error message in an appropriate way.
                // For example, logging the error might be helpful for troubleshooting.
                error_log($error_message);

                // Optionally, you can also display a user-friendly message on the website.
                echo "Oops! Something went wrong while fetching the logo image. Please try again later.";
                return false;
            }
...
//    }
    ...
    {
        error_log('bulk_disable_app_server_side_callback called'); // Add this for debugging

        if (isset($_POST["action_select"]) && in_array($_POST["action_select"], array('disabled', 'enabled'))) {
            $action = $_POST["action_select"];
            error_log('Action: ' . $action); // Add this for debugging

            $Apps = new Apps();
            $Apps->bulk_action($action,$_POST["record_ids"]);
        } else {
            $data = array(
                "success" => false,
                "status" => 400,
    ...
    public function bulk_action($action,$record_ids)
    {
        $decoded_string = stripslashes($record_ids);
        $array = json_decode($decoded_string, true);
        ...
        if (count($array) != 0) {
            foreach ($array as $recordId) {
                if ($action === 'disabled') {
                    ...
                    $this->enable_apps($recordId);
                }
            }
            $data = array(
                "success" => true,
                "status" => 200,
                "msg" => "The Apps have been " . ($action === 'disabled' ? 'disabled' : 'enabled') . " successfully."
            );
            wp_send_json($data);
        } else {
            $data = array(
    ...
    public function disable_apps($app_id)
    {
        ...
        if ($result === false) {
            $data = array(
                "success" => false,
                "status" => 500,
                "msg" => "Failed to disable the app."
            );
            wp_send_json($data);
        }
    }

    public function enable_apps($app_id)
    {
        global $wpdb;
        $table_app = $this->table_app_info;
        $result = $wpdb->update(
...
//                if ($post_id) {
//                    $wpd.b->insert($table_app_post, array(
//                        'app_id' => $app_id,
//                        'post_id' => $post_id,
//                    ));
//                    $data = array(
...
//        /*wp_send_json($data);*/    }
    ...
    public function create_post() {
        ...
        foreach ($apps as $app) {
//          $app_id = $app->api_app_id;
            $app_id = $app->id;
            $app_data_post = $wpdb->get_row("SELECT * FROM {$table_app_post} WHERE app_id = {$app_id}");

                try {
                        // Check if app_id exists in $table_app_post
                        $existing_post_id = $wpdb->get_var($wpdb->prepare("SELECT post_id FROM {$table_app_post} WHERE app_id = %d", $app_id));
                        if (!$existing_post_id) {
                            // Case 1: If the app_id doesn't exist in $table_app_post, create a new post and connect it to the app.
                            $add_posts = array(
                                'post_title' => $app->name,
                                'post_content' => 'Content of the new post',
                                'post_status' => 'publish',
                            ...
                            // Case 2: If the app_id exists in $table_app_post, check if the post_id exists in $wpdb->prefix . "posts".
                            $post_title = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$posts_table}  WHERE ID = %d AND post_status = 'publish'", $existing_post_id));

                            if (!$post_title) {
                                // Case 2.1: If the post_id doesn't exist in $wpdb->prefix . "posts", create a new post and update the connection.
                                $add_posts = array(
                                    'post_title' => $app->name,
                                    'post_content' => 'Content of the new post',
                                    'post_status' => 'publish',
                                ...
                                if ($post_id) {
                                    $wpdb->update($table_app_post, array(
                                        'post_id' => $post_id,
                                    ), array(
                                        'app_id' => $app_id,
                                    ));
